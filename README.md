...
PBHGEN.exe --check --batch first.pb second.pb
PBHGEN.exe --output generated/header.pbi source.pb
PBHGEN.exe --diagnostics json source.pb
PBHGEN.exe --version
```

`--check` compares the generated bytes without modifying the destination.
`--output` selects a destination for one source and cannot be combined with
`--batch`. Machine-readable diagnostics use JSON Lines: one compact JSON object
is emitted for each processed source, allowing a caller to consume a batch
incrementally.
...
| `1` | A checked header is missing or stale |
...
Generated headers are written to a temporary sibling file and replaced
atomically after generation succeeds. If the generated bytes are unchanged,
PBHGEN keeps the existing header and its modification time. A parser or
replacement failure therefore cannot leave a truncated header behind.
